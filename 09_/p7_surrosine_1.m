...
for i = 1:length(a)
    for j = 1:N
        ...
        temp = MkSg_Map('sine', series_length, initial_a(j), a(i), eta);
        series_matrix(j, :, i) = generate_surrogate_iaaft(temp);
    end
end
...
for i = 1:length(a)
    ...
    for j = 1:N
        ...
        AMI_features(i, j) = abs(IN_AutoMutualInfo(series_matrix(j, :, i)', 1, AMI_using_method));
...
xlabel('|AC1|')
ylabel('|AMI1|')
title('feature feature correlation for surrogate sine map')
