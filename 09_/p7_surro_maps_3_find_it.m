...
for i=1:length(series_cell)
    ...
    a = map_params{i};
    figure('Renderer', 'painters', 'Position', [10 10 800 800]) % x0,y0,width,height
    ...
    grid on
    fitted = polyval(p, AC1_matrix_abs(i, j));
    for j=1:num_A
        if abs(AMI1_matrix_surro(i, j) - fitted) > tol
            text(AC1_matrix_surro_abs(i, j), AMI1_matrix_surro(i, j), num2str(a(j)), 'FontSize', 14)
        end
    end
    l1 = "original";
    l2 = "surrogate";
