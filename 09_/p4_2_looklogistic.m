...
num_A = 2000;
a = linspace(3.57, 6, num_A);
series_length = 1000;
eta = 100;
series = NaN(num_A, series_length);

for i = 1:num_A
%     series(i,:) = MkSg_Map('logistic', series_length, 1/sqrt(2), a(i), eta);
    series(i,:) = gen_logistic(series_length, eta, a(i), 1/sqrt(2), 0.01);
end
...
plot(a,AC_features, '.')
grid on
title('logistic map AC features vs A')
xlabel('A')
ylabel('AC features')
figure
plot(a,AMI_features, '.')
grid on
title(sprintf('logistic map AMI features vs A'))
xlabel('A')
ylabel('AMI features')
