...
num_A = 600;
a = linspace(2, 5, num_A);
series_length = 1000;
eta = 100;
series = NaN(num_A, series_length);
noise_level = 0.01;
for i = 1:num_A
    series(i,:) = gen_tent_p(series_length, eta, a(i), 1/sqrt(2), noise_level);
end
...
plot(a,AC_features, '.')
title(sprintf('AC features vs A,noise level = %.3f',noise_level))
xlabel('A')
ylabel('AC features')
figure
plot(a,AMI_features, '.')
title(sprintf('AMI features vs A,noise level = %.3f',noise_level))
xlabel('A')
ylabel('AMI features')
