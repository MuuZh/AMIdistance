...
bincenters = a_AR;
SNR = linspace(25,-5,10);
% nl = db2mag(SNR);

map_accept_n = {};
for nlidx = 1:length(SNR)
    % thisnl = nl(nlidx);

    % [AC1_matrix_abs, AMI1_matrix] = p6_get_features(series_length,thisnl);
    [AC1_matrix_abs, AMI1_matrix] = get_features_by_SNR(series_length, SNR(nlidx));
...
T = table(SNR', accept_rate(:,1), accept_rate(:,2), accept_rate(:,3), accept_rate(:,4), accept_rate(:,5), accept_rate(:,6), 'VariableNames', map_names);

% plot the accept rate for each map
figure

hold on
plot(SNR, accept_rate(:,1), 'LineWidth', 2)
plot(SNR, accept_rate(:,2), 'LineWidth', 2)
plot(SNR, accept_rate(:,3), 'LineWidth', 2)
plot(SNR, accept_rate(:,4), 'LineWidth', 2)
plot(SNR, accept_rate(:,5), 'LineWidth', 2)
plot(SNR, accept_rate(:,6), 'LineWidth', 2)
...
% compute rejection rate for each map
rejection_rate = 1 - accept_rate;

rSNR = SNR(end:-1:1);
hold on
plot(SNR, rejection_rate(:,1), 'LineWidth', 2)
plot(SNR, rejection_rate(:,2), 'LineWidth', 2)
plot(SNR, rejection_rate(:,3), 'LineWidth', 2)
plot(SNR, rejection_rate(:,4), 'LineWidth', 2)
plot(SNR, rejection_rate(:,5), 'LineWidth', 2)
plot(SNR, rejection_rate(:,6), 'LineWidth', 2)
legend('sine', 'logistic', 'tent' , 'AR', 'tent with noise', 'sine with more periods')
xlabel('SNR (dB)')
