...
logistic_noise_result = np.array(logistic_noise_result)
# Replace nan and inf values with zeros
logistic_noise_result[mask] = 0
...
#%% bookkeeping
l_AMIdistance_noise_result = logistic_noise_result
l_AMIdistance_LE_correlation_noise = logistic_correlation
l_AMIdistance_time_noise = runningTime
...
logistic_correlation = np.array([
    np.corrcoef(logisticLambdas, lambda_e_noise_result[i])[0, 1]
    for i in range(len(snr))
    ])
pickedidx = [0, len(snr)-1]
plt.figure(figsize=(20, 5))
...
for idx, i in enumerate(pickedidx):
    l = snr[i]
    plt.subplot(1, len(pickedidx), idx+1)
    plt.scatter(logisticLambdas, lambda_e_noise_result[i], marker='.',alpha=0.5)
    plt.xlabel("ground true LE")
    plt.ylabel("logistic AMI distance")
    plt.title("Eckmann, SNR = {}, correlation = {:.3f}".format(l, logistic_correlation[i]))
plt.show()
...
#%% bookkeeping
l_lambda_e_noise_result = lambda_e_noise_result
l_lambda_e_correlation_noise = logistic_correlation
l_lambda_e_time_noise = runningTime

l_varying_noise = snr
l_varying_noise_parameter = [x_start, series_length, logistic_num]

#%% save the result
np.savez('logistic_varying_noise.npz',
            l_AMIdistance_noise_result = l_AMIdistance_noise_result,
            l_AMIdistance_LE_correlation_noise = l_AMIdistance_LE_correlation_noise,
            l_AMIdistance_time_noise = l_AMIdistance_time_noise,
            l_lambda_e_noise_result = l_lambda_e_noise_result,
            l_lambda_e_correlation_noise = l_lambda_e_correlation_noise,
            l_lambda_e_time_noise = l_lambda_e_time_noise,
            l_varying_noise = l_varying_noise,
            l_varying_noise_parameter = l_varying_noise_parameter
            )