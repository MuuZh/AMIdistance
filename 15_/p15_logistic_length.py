...
for i, l in tenumerate(segemnt_length):
    ...
    series_segment = logistic_full_data[:, :l]

    t0 = time.time()
    logisticAMI1 = automutual_info(series_segment, 1, 3)
    ...
    logisticAMI1_AR1 = AMI_for_AR1_all_at_once(logisticAC1, sample_size=50, series_length=l)
...
for idx, i in enumerate(pickedidx):
    ...
    plt.title("length = {}, correlation = {:.3f}".format(l, logistic_correlation[i]))
plt.show()
...
plt.figure()
plt.plot(segemnt_length, logistic_correlation, label="correlation")
plt.xlabel("length")
...
l_AMIdistance_length_result = logistic_length_result
l_AMIdistance_LE_correlation_length = logistic_correlation
l_AMIdistance_time_length = runningTime
...
#%% bookkeeping
# segemnt_length = segemnt_length
l_lambda_e_length_result = lambda_e_length_result
l_lambda_e_correlation_length = logistic_correlation
l_lambda_e_time_length = runningTime

l_varying_length = segemnt_length
l_varying_length_prameter = [x_start, series_length, logistic_num]
